...
from tests.utils import upload_topography_file
...
def test_list_surface_permissions(api_client, handle_usage_statistics):
    ...
    assert response.data["permissions"]["current_user"][
        "user"
    ] == get_api_url(user1, response.wsgi_request)
    ...
    for permissions in other_permissions:
        if permissions["user"] == get_api_url(user2, response.wsgi_request):
            # related to user 2
            assert permissions["permission"] == "view"
        elif permissions["user"] == get_api_url(user3, response.wsgi_request):
            # related to user 3
